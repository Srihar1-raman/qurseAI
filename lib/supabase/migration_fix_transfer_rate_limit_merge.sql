-- =====================================================
-- MIGRATION: Fix transfer_guest_to_user rate limit merge
-- Fixes constraint violation when user already has rate limit for same bucket
-- Safe to run multiple times (CREATE OR REPLACE)
-- =====================================================

-- Fix transfer_guest_to_user to merge rate limits when user already has record
...
-- 
-- 1. Create guest data with rate limit
-- 2. Create user with existing rate limit for same bucket
-- 3. Call transfer_guest_to_user()
-- 4. Verify:
--    - Guest conversations transferred
--    - Guest messages transferred
--    - Rate limits merged (user count = old user count + guest count)
